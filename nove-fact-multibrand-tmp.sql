...
SELECT
  ...
  sum(coupon_amount) as total_coupon_amount,
     count (distinct(case when cast(coupon_amount as float64)>0 then contact_email end)) as num_customer_with_coupon,

  count (distinct(case when coupon_amount>0 then contact_email end)) / count(distinct contact_email) as num_customer_with_coupon_ratio,
  sum(case when coupon_amount>0 then 1 else 0 end) / count(booking_id) as booking_with_coupon_ratio,
  ...
  sum(total_fare) / count(distinct contact_email) as spend_per_customer,
  ...
  case when sum(coupon_amount) > 0 then sum(total_fare)*1.0 / sum(coupon_amount) else 0 end as ROI
...
SELECT
  ...
  sum(total_fare) as total_gbv,
   ...
   count (distinct(case when cast(coupon_amount as float64)>0 then contact_email end)) as num_customer_with_coupon,
 count (distinct(case when cast(coupon_amount as float64)>0 then contact_email end)) / count(distinct contact_email) as num_customer_with_coupon_ratio,
  sum(case when cast(coupon_amount as float64)>0 then 1 else 0 end) / count(sales.booking_id) as booking_with_coupon_ratio,
  ...
  sum(num_adult + num_child + num_infant) / count(distinct contact_email) as pax_per_customer,
  case when sum(cast(coupon_amount as float64)) > 0 then sum(total_fare)*1.0 / sum(cast(coupon_amount as float64)) else 0 end as ROI
