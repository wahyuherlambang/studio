...
SELECT
  'Airy' as brand,
  date_trunc(date(issued_date), week) as week,
  ...
  sum(total_purchase_price)*1.0 / sum(coupon_amount) as ROI
...
SELECT
  ...
  sum(num_adult + num_child + num_infant) / count(distinct contact_email) as pax_per_customer,
  sum(total_purchase_price)*1.0 / sum(cast (coupon_amount as float64)) as ROI
...
WHERE date(ISSUED_DATE) <= date("2018-07-31") and
